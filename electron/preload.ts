...
const electronAPI = {
  ...
  processTextQuery: (query: string) =>
    ipcRenderer.invoke("process-text-query", query),
  onToggleTextInput: (callback: () => void) => {
    const subscription = () => callback();
    ipcRenderer.on("toggle-text-input", subscription);
    return () => {
      ipcRenderer.removeListener("toggle-text-input", subscription);
    };
  },
  removeToggleTextInputListener: (callback: () => void) => {
    ipcRenderer.removeListener("toggle-text-input", callback);
  },
}
