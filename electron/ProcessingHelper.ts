...
export class ProcessingHelper {
  ...
  public async processTextQuery(query: string): Promise<void> {
    const mainWindow = this.deps.getMainWindow();
    if (!mainWindow) return;
    ...
    // First verify we have a valid AI client
    if (
      (config.apiProvider === "openai" && !this.openaiClient) ||
    ...
    try {
      ...
      // Create a simplified problemInfo from the text query
      const problemInfo = {
        problem_statement: query,
        constraints: "Not specified.",
        example_input: "Not specified.",
        example_output: "Not specified.",
      };
        ...
        const messages = [
          {
            ...
            content:
              "You are an expert OCR and coding challenge interpreter. Your task is to meticulously analyze the provided screenshot(s) of a coding problem and extract all relevant information with the highest possible accuracy. Pay close attention to details, including mathematical formulas, code snippets, and edge cases. Return the extracted information in a structured JSON format with the following fields: problem_statement, constraints, example_input, example_output. Do not add any explanatory text outside of the JSON object. Ensure the JSON is well-formed.",
          },
          {
            role: "user" as const,
            content: [
        ...
        try {
          ...
          const geminiMessages: GeminiMessage[] = [
            {
              ...
              parts: [
                {
                  text: `You are an expert OCR and coding challenge interpreter. Your task is to meticulously analyze the provided screenshot(s) of a coding problem and extract all relevant information with the highest possible accuracy. Pay close attention to details, including mathematical formulas, code snippets, and edge cases. Return the extracted information in a structured JSON format with the following fields: problem_statement, constraints, example_input, example_output. Do not add any explanatory text outside of the JSON object. Ensure the JSON is well-formed. The preferred coding language for the solution is ${language}.`,
                },
                ...imageDataList.map((data) => ({
                  inlineData: {
                    mimeType: "image/png",
          ...
          if (
            ...
            if (responseData.candidates?.[0]?.finishReason === "SAFETY") {
              throw new Error(
                "Request blocked by Gemini API for safety reasons. Please adjust your query."
              );
            }
...
const maxTokens = language === 'html' ? 8192 : 4000;
      ...
      const thoughtsRegex =
        /(?:Thoughts:|Key Insights:|Reasoning:|Approach:)([\s\S]*)/i;
