...
export function Header({
  ...
  useEffect(() => {
    const handleToggle = () => setTextInputVisible(prev => !prev);
    const unsubscribe = window.electronAPI.onToggleTextInput(handleToggle);
    return () => {
      unsubscribe();
    };
  }, [setTextInputVisible]);

  const handleTextQuerySubmit = () => {
    if (textQuery.trim()) {
      window.electronAPI.processTextQuery(textQuery);
      setTextQuery("");
        ...
        {isTextInputVisible && (
          <div className="flex items-center space-x-2 mr-4">
            <input
              ...
              onKeyDown={(e) => e.key === "Enter" && handleTextQuerySubmit()}
              placeholder="Enter your query..."
              className="bg-white/10 text-white px-2 py-1 rounded-md text-sm"
            />
            <Button
              size="sm"
              className="h-8 px-2 bg-blue-500 text-white hover:bg-blue-600"
